...
import org.bsu.webencryptor.api.controller.ECBController;
...
public class ECBControllerImpl implements ECBController {

    private final ControllerUtils controllerUtils;
    private final ECBService ecbService;

    @Override
    public ResponseEntity<?> encryptBeltEcb(EncryptRequest req) {
        try {
            byte[] key = controllerUtils.decodeBase64(req.getKeyBase64());
            byte[] plain = req.getPlaintext().getBytes(StandardCharsets.UTF_8);
            String ciphertextB64 = ecbService.encryptBeltEcbBytes(key, plain);
    ...
    @Override
    public ResponseEntity<?> decryptBeltEcb(DecryptRequest req) {
        try {
            byte[] key = controllerUtils.decodeBase64(req.getKeyBase64());
            byte[] ciphertext = controllerUtils.decodeBase64(req.getCiphertextBase64());
            byte[] decryptedBytes = ecbService.decryptBeltEcbBytes(key, ciphertext);
    ...
    @Override
    public ResponseEntity<?> encryptFileStreamBeltEcb(String keyBase64, MultipartFile file) {
        try (InputStream in = file.getInputStream()) {
            byte[] key = controllerUtils.decodeBase64(keyBase64);
            File enc = ecbService.encryptFileStreamBeltEcb(key, in);
    ...
    @Override
    public ResponseEntity<?> decryptFileStreamBeltEcb(String keyBase64, MultipartFile file) {
        try (InputStream in = file.getInputStream()) {
            byte[] key = controllerUtils.decodeBase64(keyBase64);
            File out = ecbService.decryptFileStreamBeltEcb(key, in);
