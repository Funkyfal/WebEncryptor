...
import org.bsu.webencryptor.api.controller.CBCController;
...
public class CBCControllerImpl implements CBCController {

    private final ControllerUtils controllerUtils;
    private final CBCService cbcService;

    @Override
    public ResponseEntity<?> encryptBeltCbc(EncryptRequest req) {
        try {
            ...
            String ciphertextB64 = cbcService.encryptBeltCbcBytes(key, iv, plain);
    ...
    @Override
    public ResponseEntity<?> decryptBeltCbc(DecryptRequest req) {
        try {
            byte[] key = controllerUtils.decodeBase64(req.getKeyBase64());
            byte[] iv  = req.getIvBase64() == null ? null : controllerUtils.decodeBase64(req.getIvBase64());
            byte[] ciphertext = controllerUtils.decodeBase64(req.getCiphertextBase64());
            byte[] decryptedBytes = cbcService.decryptBeltCbcBytes(key, iv, ciphertext);
    ...
    @Override
    public ResponseEntity<?> encryptFileStreamBeltCbc(String keyBase64, String ivBase64, MultipartFile file) {
        try (InputStream in = file.getInputStream()) {
            byte[] key = controllerUtils.decodeBase64(keyBase64);
            byte[] iv = controllerUtils.decodeBase64(ivBase64);
            File enc = cbcService.encryptFileStreamBeltCbc(key, iv, in);
    ...
    @Override
    public ResponseEntity<?> decryptFileStreamBeltCbc(String keyBase64, String ivBase64, MultipartFile file) {
        try (InputStream in = file.getInputStream()) {
            byte[] key = controllerUtils.decodeBase64(keyBase64);
            byte[] iv = controllerUtils.decodeBase64(ivBase64);
            File out = cbcService.decryptFileStreamBeltCbc(key, iv, in);
