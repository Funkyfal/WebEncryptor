...
public class HashControllerImpl implements HashController {
    ...
    @Override
    public ResponseEntity<?> belt(HashRequest req) {
        try {
            byte[] data = req.getData() == null ? new byte[0] : req.getData().getBytes(java.nio.charset.StandardCharsets.UTF_8);
            String b64 = hashService.hashBeltBase64(data);
            String hex = hashService.hashBeltHex(data);
            return ResponseEntity.ok(Map.of("belt_base64", b64, "belt_hex", hex));
        } catch (RuntimeException e) {
            return ResponseEntity.status(500).body(Map.of("error", e.getMessage()));
        }
    }
    ...
    @Override
    public ResponseEntity<?> beltFile(MultipartFile file) {
        try (InputStream in = file.getInputStream()) {
            byte[] hash = hashService.hashBeltFileStream(in);
            String b64 = Base64.getEncoder().encodeToString(hash);
            String hex = bytesToHex(hash);
            return ResponseEntity.ok(Map.of("belt_base64", b64, "belt_hex", hex, "len", hash.length));
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("error", e.getMessage()));
        }
    }
    ...
    @Override
    public ResponseEntity<?> verifyBeltFile(MultipartFile file, String hashBase64) {
        try (InputStream in = file.getInputStream()) {
            boolean valid = hashService.verifyBeltFile(in, hashBase64);
            return ResponseEntity.ok(Map.of("valid", valid));
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("error", e.getMessage()));
        }
